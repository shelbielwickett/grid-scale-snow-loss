...
async def worker(year, n, site_dict, semaphore, total_requests, session, request_times, lock):
    ...
    site_file = f'/Users/shelbiedavis1/Multi-State Simulation/NSRDB_parquet/{year+1}_NSRDB_parquet/{lat}_{lon}.parquet'
    ...
    async with semaphore:
        try:
            ...
            # Convert 'timestamp' (local time) to UTC for filtering
            df['UTC'] = pd.to_datetime(df['timestamp']) - pd.to_timedelta(metadata['Time Zone'], unit='h')

            # Define UTC range
            utc_start = pd.Timestamp("2022-01-01 00:00:00", tz="UTC")
            utc_end = pd.Timestamp("2023-01-01 00:00:00", tz="UTC")

            # Filter to keep only rows with UTC timestamps in the 2022 range
            df_filtered = df.set_index('UTC').loc[utc_start:utc_end - pd.Timedelta(hours=1)].reset_index(drop=True)

            # Log and skip saving if empty
            if df_filtered.empty:
                print(f"⚠️ No UTC-aligned data in 2022 for project {project_name}")
                write_to_log(f"⚠️ No UTC-aligned data in 2022 for project {project_name} (lat: {lat}, lon: {lon})")
                return None

            # Drop UTC column only if it exists
            df_filtered.drop(columns=['UTC'], errors='ignore', inplace=True)

            # Save filtered data
            async with lock:
                save_to_parquet(df_filtered, site_file, metadata)
            return structured_data
